...
public class ShopPublicController {
    ...
    public ResponseEntity<ShopInfoResponse> getShopInfo(
            @PathVariable @Parameter(description = "Идентификатор магазина", required = true, example = "123e4567-e89b-12d3-a456-426614174000")
            UUID shopId
    ) {
    ...
    public ResponseEntity<List<ProductInfoInternResponse>> getShopProducts(
            @PathVariable @Parameter(description = "Идентификатор магазина", required = true, example = "123e4567-e89b-12d3-a456-426614174000")
            UUID shopId,
            @RequestParam(name = "page", defaultValue = "0") @Parameter(description = "Номер страницы (начиная с 0)", example = "0") int page,
            @RequestParam(name = "size", defaultValue = "10") @Parameter(description = "Количество элементов на странице", example = "10") int size
    ) {
        return ResponseEntity.ok(productsService.getInternProducts(shopId.toString(), page, size));
    }
}
